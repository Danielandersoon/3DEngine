...
Engine::Engine(unsigned int x, unsigned int y, std::string title) :
	screenX(x),
	screenY(y)
{
	app.create(sf::VideoMode(x, y), title);
...
Vector3 Engine::Matrix_MultiplyVector(Matrix4x4& m, Vector3& i)
{
	Vector3 v;
...
Matrix4x4 Engine::Matrix_MakeIdentity()
{
	Matrix4x4 matrix;
...
Matrix4x4 Engine::Matrix_MakeRotationX(float fAngleRad)
{
	Matrix4x4 matrix;
	matrix.matrix[0][0] = 1.0f;
...
Matrix4x4 Engine::Matrix_MakeRotationY(float fAngleRad)
{
	Matrix4x4 matrix;
...
Matrix4x4 Engine::Matrix_MakeRotationZ(float fAngleRad)
{
	Matrix4x4 matrix;
	matrix.matrix[0][0] = cosf(fAngleRad);
...
Matrix4x4 Engine::Matrix_MakeTranslation(float x, float y, float z)
{
	Matrix4x4 matrix;
	matrix.matrix[0][0] = 1.0f;
	matrix.matrix[1][1] = 1.0f;
...
Matrix4x4 Engine::Matrix_MakeProjection(float fFovDegrees, float fAspectRatio, float fNear, float fFar)
{
...
Matrix4x4 Engine::Matrix_MultiplyMatrix(Matrix4x4& m1, Matrix4x4& m2)
{
	Matrix4x4 matrix;
	for (int c = 0; c < 4; c++)
		for (int r = 0; r < 4; r++)
...
Matrix4x4 Engine::Matrix_PointAt(Vector3& pos, Vector3& target, Vector3& up)
{
...
Matrix4x4 Engine::Matrix_QuickInverse(Matrix4x4& m) // Only for Rotation/Translation Matrices
{
	Matrix4x4 matrix;
...
Vector3 Engine::Vector_Add(Vector3& v1, Vector3& v2)
{
	return { v1.x + v2.x, v1.y + v2.y, v1.z + v2.z };
}

Vector3 Engine::Vector_Sub(Vector3& v1, Vector3& v2)
{
	return { v1.x - v2.x, v1.y - v2.y, v1.z - v2.z };
}

Vector3 Engine::Vector_Mul(Vector3& v1, float k)
{
	return { v1.x * k, v1.y * k, v1.z * k };
}

Vector3 Engine::Vector_Div(Vector3& v1, float k)
{
	return { v1.x / k, v1.y / k, v1.z / k };
}

float Engine::Vector_DotProduct(Vector3& v1, Vector3& v2)
{
	return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

float Engine::Vector_Length(Vector3& v)
{
	return sqrtf(Vector_DotProduct(v, v));
}

Vector3 Engine::Vector_Normalise(Vector3& v)
{
	float l = Vector_Length(v);
	return { v.x / l, v.y / l, v.z / l };
}

Vector3 Engine::Vector_CrossProduct(Vector3& v1, Vector3& v2)
{
	Vector3 v;
...
Vector3 Engine::Vector_IntersectPlane(Vector3& plane_p, Vector3& plane_n, Vector3& lineStart, Vector3& lineEnd)
{
...
int Engine::Triangle_ClipAgainstPlane(Vector3 plane_p, Vector3 plane_n, Triangle& in_tri, Triangle& out_tri1, Triangle& out_tri2)
{
	plane_n = Vector_Normalise(plane_n);
...
bool Engine::OnUserUpdate(sf::Event event)
{
	if (event.KeyPressed == sf::Keyboard::Left)
		vCamera.x -= 8.0f * dt;	// Travel Along X-Axis

	if (event.KeyPressed == sf::Keyboard::Right)
		vCamera.x += 8.0f * dt;	// Travel Along X-Axis


	Vector3 vForward = Vector_Mul(vCameraDirection, 8.0f * dt);
	...
	for (auto& triToRaster : vecTrianglesToRaster)
	{
		...
		{
			//FillTriangle(t.vect[0].x, t.vect[0].y, t.vect[1].x, t.vect[1].y, t.vect[2].x, t.vect[2].y, t.sym, t.col);
